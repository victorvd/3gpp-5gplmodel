...
$(document).ready(function () {
    ...
    $("#frm_parm").submit(function () {
        ...
        $.post("calc_UMa.php", datosForm, mostrResul);
...
function mostrResul(table) {
    ...
    if (table[0][0] !== "[") {
        ...
        var PLUMa = jQuery.parseJSON(table);
        var ite = PLUMa.length - 1;
    ...
    $("#txt_plos").val(PLUMa[ite][1].toFixed(1));
    $("#txt_shwf").val(PLUMa[ite][2].toFixed(1));
    $("#txt_plsf").val((PLUMa[ite][1] + PLUMa[ite][2]).toFixed(1));
    ...
    for (var i = ite; i >= 0; i--) {
        ...
        fila.insertCell(1).innerHTML = (+(PLUMa[i][0])).toFixed(2);
        fila.insertCell(2).innerHTML = parseFloat(PLUMa[i][1]).toFixed(2);
    }
    ...
    //Seleccionar columnas 1 y 2 del array PLUMa
    var col3x = PLUMa.map(function (value, index) {
        return value[0];
    });
    var col3y = PLUMa.map(function (value, index) {
        return value[1];
    });
