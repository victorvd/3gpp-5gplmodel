...
$(document).ready(function () {
    ...
    $("#frm_parm").submit(function () {
        ...
        $.post("calc_UMi.php", datosForm, mostrResul);
...
function mostrResul(table) {
    ...
    if (table[0][0] !== "[") {
        ...
        var PLUMi = jQuery.parseJSON(table);
        var ite = PLUMi.length - 1;
    ...
    $("#txt_plos").val(PLUMi[ite][1].toFixed(1));
    $("#txt_shwf").val(PLUMi[ite][2].toFixed(1));
    $("#txt_plsf").val((PLUMi[ite][1] + PLUMi[ite][2]).toFixed(1));
    ...
    for (var i = ite; i >= 0; i--) {
        ...
        fila.insertCell(1).innerHTML = (+(PLUMi[i][0])).toFixed(2);
        fila.insertCell(2).innerHTML = parseFloat(PLUMi[i][1]).toFixed(2);
    }
    ...
    //Seleccionar columnas 1 y 2 del array PLUMi
    var col3x = PLUMi.map(function (value, index) {
        return value[0];
    });
    var col3y = PLUMi.map(function (value, index) {
        return value[1];
    });
