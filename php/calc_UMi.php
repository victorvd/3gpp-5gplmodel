...
try {
    ...
    switch ($_POST["txt_tab"]) {            //Opciones para tabular
        case "opt_unic":                    //Opción un único valor
            ...
            list($PLUMi[0], $sdSF[0]) = pathloss($dar[0], $dBP[0], $d3D[0], $far[0], $hBS, $hUT);
...
//            } else {
            ...
            for ($i = 0; $i < $ite; $i++) {
                ...
                list($PLUMi[$i], $sdSF[$i]) = pathloss($dar[0], $dBP[$i], $d3D[0], $far[$i],$hBS, $hUT);
            }
//            }
            break;
...
//            } else {
            ...
            for ($i = 0; $i < $ite; $i++) {
                ...
                list($PLUMi[$i], $sdSF[$i]) = pathloss($dar[$i], $dBP[0], $d3D[$i], $far[0], $hBS, $hUT);
            }
//            }
            $far = $d3D;
            break;
    ...
    $table = unArreglo($far, $PLUMi, $sdSF);
...
function pathloss($dar, $dBP, $d3D, $far, $hBS, $hUT) {
    if ($dar <= $dBP && $dar >= 10) {
        $PLUMi = 32.4 + 21 * log10($d3D) + 20 * log10($far);
        $sdSF = normalDistribution(0, 4);
    } else if ($dar <= 5000 && $dar >= $dBP) {
        $PLUMi = 32.4 + 40 * log10($d3D) + 20 * log10($far) - 9.5 * log10(pow($dBP, 2) + pow($hBS - $hUT,2));
        $sdSF = normalDistribution(0, 4);
    } else {
    ...
    if ($_POST["txt_los"] === "opt_nlos") {
        ...
        if ($dar <= 5000 && $dar >= 10) {
            $PLUMi_p = 22.4 + 35.3 * log10 ($d3D) + 21.3 * log10($far) - 0.3 * ($hUT - 1.5);
            $PLUMi = max($PLUMi, $PLUMi_p);
            $sdSF = normalDistribution(0, 7.82);
        } else {
